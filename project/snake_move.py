...
field_array = [[16] * 11] #0:empty, 1:player, 2:enemy, 4:apple, 8:bomb, 16:wall, 32:head
for i in range(0, 15):
...
class apple():
    def __init__(self, gx, gy):
        ...
        self.exist = True
        field_array[gx+1][gy+1] |= field_dict['apple']
    def draw(self):
        if(self.exist):
            self.image.draw(self.x, self.y)
        else: return
...
def handle_events():
    ...
    for event in events:
        if event.type == SDL_QUIT:
            ...
            acting = False
        elif event.type == SDL_KEYDOWN and event.key == SDLK_w and cur_direction not in (1,3,4):
            direction = 1
        elif event.type == SDL_KEYDOWN and event.key == SDLK_a and cur_direction not in (0,2,4):
            direction = 2
        elif event.type == SDL_KEYDOWN and event.key == SDLK_s and cur_direction not in (1,3,4):
            direction = 3
        elif event.type == SDL_KEYDOWN and event.key == SDLK_d and cur_direction not in (0,2,4):
            direction = 0
